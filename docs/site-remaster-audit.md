...
The next contrasting target is Applied Science → Architecture. Audit its ontology, site/program/structure/material/environment relationships, vocabulary ownership, and whether its primary model should be spatial, sectional, or constraint-driven. Avoid importing Music's mixer, waveform, or recording grammar.
