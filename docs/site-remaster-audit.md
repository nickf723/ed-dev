...
The next contrasting target is Natural Science → Biology → Botany. Preserve any successful biological process models while auditing its bounded learning contract, curriculum context, vocabulary ownership, collection opportunity, and whether the existing iNaturalist adapter can support plant discovery with honest provider and image provenance.
