...
The next contrasting target is Formal Science → Computer Science. Audit whether its current root cleanly separates theory, hardware, software, algorithms, AI, and security without turning the discipline into a generic neon terminal. Preserve strong computational instruments, derive navigation and vocabulary from the registry, and establish one tested model in a visual grammar unlike Anthropology's field record.
