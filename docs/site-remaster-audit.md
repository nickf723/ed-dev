...
The next contrasting target is Applied Science → Materials Science. Audit whether its existing ontology separates material classes, structure, processing, properties, characterization, selection, and failure without turning reference specimens into unsupported materials advice. Preserve any accurate interactives, but give the subject a physical test-bench grammar rather than importing Data Science's analytical panels.
