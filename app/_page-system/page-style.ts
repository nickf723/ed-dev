...
export const SECTION_GAP = { sm: "mt-3", md: "mt-5", lg: "mt-8" } as const;
export const RADIUS = { md: "18px", lg: "24px", xl: "32px" } as const;
...
export const REGIME_COLUMNS = { 1: "xl:grid-cols-1", 2: "xl:grid-cols-2" } as const;
export const ITEM_COLUMNS = { 1: "grid-cols-1", 2: "sm:grid-cols-2" } as const;

export function pageBackground(recipe: PageRecipe) {
  return recipe.theme.resolvedPalette
    ? `rgb(${recipe.theme.resolvedPalette.background})`
...
export function surfaceColor(recipe: PageRecipe) {
  ...
  const rgb = recipe.theme.resolvedPalette?.surface ??
    (recipe.theme.family === "history" ? "10, 7, 5" : "3, 7, 13");
  return `rgba(${rgb},${opacity})`;
}

export function headerColor(recipe: PageRecipe) {
  const rgb = recipe.theme.resolvedPalette?.background ??
    (recipe.theme.family === "history" ? "7, 5, 3" : "3, 7, 13");
  return `rgba(${rgb},${Math.min(0.94, 0.58 + recipe.theme.surfaceOpacity)})`;
}
...
export function displayFontClass(recipe: PageRecipe) {
  const font = recipe.theme.displayFont ?? (recipe.theme.family === "history" ? "serif" : "mono");
  return font === "serif" ? "font-serif" : font === "mono" ? "font-mono" : "font-sans";
}
export function bodyFontClass(recipe: PageRecipe) {
  const font = recipe.theme.bodyFont ?? "sans";
  return font === "serif" ? "font-serif" : font === "mono" ? "font-mono" : "font-sans";
}
export function labelFontClass(recipe: PageRecipe) {
  return recipe.theme.displayFont === "serif" ? "font-sans" : "font-mono";
}
export function headingClass(recipe: PageRecipe) {
  return `${displayFontClass(recipe)} ${recipe.theme.titleCase === "uppercase" ? "uppercase" : ""}`;
}
...
export function titleClass(recipe: PageRecipe) {
  ...
  const titleCase = recipe.theme.titleCase ?? (recipe.theme.family === "history" ? "natural" : "uppercase");
  const casing = titleCase === "uppercase" ? "uppercase" : "";
  const color = recipe.theme.family === "history" ? "text-[#fffaf0]" : "text-[#f7fbff]";
  if (scale === "compact") return `${font} ${casing} text-[clamp(2rem,3.8vw,4.8rem)] font-semibold leading-[0.9] tracking-[-0.05em] ${color}`;
  if (scale === "display") return `${font} ${casing} text-[clamp(3.1rem,6vw,7rem)] font-semibold leading-[0.8] tracking-[-0.064em] ${color}`;
  return `${font} ${casing} text-[clamp(2.6rem,5vw,6rem)] font-semibold leading-[0.84] tracking-[-0.058em] ${color}`;
}
