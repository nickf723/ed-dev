  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-end">
      <button
        ...
        onClick={onClose}
        className="absolute inset-0 bg-black/[0.78] backdrop-blur-sm"
      />
      <aside
        ref={dialogRef}
        role="dialog"
        ...
        className="relative z-10 h-full w-full max-w-[620px] overflow-y-auto border-l border-white/[0.09] bg-[#08070d]/[0.97] shadow-[-30px_0_100px_rgba(0,0,0,0.45)]"
      >
        <div className="sticky top-0 z-20 flex items-center justify-between border-b border-white/[0.08] bg-[#08070d]/90 px-5 py-4 backdrop-blur-xl">
          ...
          <button
            ...
            onClick={onClose}
            className="flex h-9 w-9 items-center justify-center rounded-full border border-white/[0.08] text-slate-600 hover:text-white"
          >
            <X size={15} />
          </button>
        </div>
        ...
        <div className="p-5 sm:p-7">
          <div className="grid gap-5 sm:grid-cols-[190px_minmax(0,1fr)]">
            ...
            <div className="self-end">
              ...
              <h2 className="mt-1 text-[clamp(2rem,5vw,3.7rem)] font-semibold leading-[0.92] tracking-[-0.055em] text-white">
                {record.title}
              </h2>
              ...
              {record.subtitle ? (
                <p className="mt-1 font-serif text-[12px] italic text-slate-500">
                  {record.subtitle}
                </p>
              ) : null}
            </div>
          ...
          {record.description ? (
            <p className="mt-7 text-[13px] leading-6 text-slate-400">
              {record.description}
            </p>
          ) : null}
          ...
          {record.tags.length ? (
            <div className="mt-5 flex flex-wrap gap-1.5">
              {record.tags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full border border-white/[0.07] bg-white/[0.018] px-2.5 py-1 text-[11px] text-slate-500"
                >
                  {tag}
                </span>
              ))}
          ...
          <section className="mt-8">
            <div className="font-mono text-[10px] font-semibold uppercase tracking-[0.10em] text-slate-500">
              Record facts
            </div>
            <div className="mt-3 divide-y divide-white/[0.06] rounded-[16px] border border-white/[0.07] bg-white/[0.014] px-4">
              {Object.entries(record.facts)
                ...
                .map(([label, value]) => (
                  <div
                    key={label}
                    className="grid grid-cols-[130px_minmax(0,1fr)] gap-4 py-3 text-[11px]"
                  >
                ...
                {record.sources.map((source) =>
                  source.url ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-2 rounded-full border border-white/[0.08] bg-white/[0.018] px-3 py-2 text-[11px] text-slate-500 hover:text-white"
                    >
                      {source.label}
                    ...
                    <span
                      key={source.label}
                      className="rounded-full border border-white/[0.07] px-3 py-2 text-[11px] text-slate-500"
                    >
                      {source.label}
                    </span>
                  )
