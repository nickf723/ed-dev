  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden" aria-hidden="true">
      ...
      <svg viewBox="0 0 1400 900" preserveAspectRatio="none" className="absolute inset-0 h-full w-full opacity-75">
        {THREADS.map((thread) => (
          <g key={thread.y} style={{ animation: `dialectic-drift ${28 + thread.phase * 6}s ease-in-out infinite`, animationDelay: `${-thread.phase * 4}s` }}>
            <path
        ...
        {[
          ...
          <g key={index} style={{ animation: `dialectic-pulse ${9 + index * 1.7}s ease-in-out infinite`, animationDelay: `${-index * 1.4}s` }}>
            <circle cx={Number(cx)} cy={Number(cy)} r="5" fill={`rgba(${rgb},0.30)`} />
            <circle cx={Number(cx)} cy={Number(cy)} r="26" fill="none" stroke={`rgba(${rgb},0.07)`} />
          </g>
        ))}
      </svg>
      ...
      <div
        ...
        style={{
          backgroundImage: "linear-gradient(rgba(251,191,36,0.028) 1px,transparent 1px),linear-gradient(90deg,rgba(192,132,252,0.028) 1px,transparent 1px)",
