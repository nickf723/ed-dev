...
import {
  ...
  NetworkLens,
  TimelineLens,
} from "@/app/_page-system/RecipeDiagrams";
...
export default function LensTopology({
  ...
  return (
    <>
      <section
        className={`${SECTION_GAP[recipe.theme.sectionGap]} flex flex-col gap-2 px-1 sm:flex-row sm:items-end sm:justify-between`}
      >
        <div>
          <div
          ...
          <h2
            className={`mt-1 text-[clamp(1.7rem,3vw,2.55rem)] text-white ${headingClass(recipe)}`}
          >
            {organization.title}
          </h2>
        </div>
...
function LensCard({
  ...
  const body = (
    <article
      ...
      style={{
        ...
        borderColor: `rgba(${item.accentRgb},${
          live ? borderAlpha(recipe, 0.18) : borderAlpha(recipe, 0.08)
        })`,
    ...
    >
      <div
        ...
        style={{
          background: `radial-gradient(circle at 52% 40%, rgba(${item.accentRgb},0.11), transparent 58%)`,
        }}
      />
      <div className="relative z-10 flex items-start justify-between gap-4">
        <div>
          <div
            className={`${labelFontClass(recipe)} text-[9px] font-semibold uppercase tracking-[0.16em]`}
            style={{ color: `rgba(${item.accentRgb},0.70)` }}
          >
            {String(index + 1).padStart(2, "0")} · {item.question}
          </div>
          <h3 className={`mt-2 text-[25px] text-white ${headingClass(recipe)}`}>
            {item.label}
          </h3>
        </div>
        <div
      ...
      <p className="relative z-10 mt-3 min-h-[72px] text-[12px] leading-6 text-slate-400">
        {item.summary}
      </p>
      <div className="relative z-10 mt-5 flex-1">
        {item.visual === "timeline" ? (
          <TimelineLens accentRgb={item.accentRgb} />
        ) : null}
        {item.visual === "map" ? (
          <MapLens accentRgb={item.accentRgb} />
        ) : null}
        {item.visual === "network" ? (
          <NetworkLens accentRgb={item.accentRgb} />
        ) : null}
      </div>
      <div className="relative z-10 mt-5 flex items-center justify-between border-t border-white/[0.06] pt-4">
        <span
          className={`${labelFontClass(recipe)} text-[9px] uppercase tracking-[0.12em]`}
        ...
        {live ? (
          <ArrowRight
            ...
            style={{ color: `rgba(${item.accentRgb},0.72)` }}
          />
        ) : (
          <CircleDashed
            size={15}
  ...
  return (
    <RecipeLinkFrame
      ...
      preview={preview}
      onSelect={
        onSelect ? () => onSelect({ kind: "lens", id: item.id }) : undefined
      }
    >
      {body}
    </RecipeLinkFrame>
