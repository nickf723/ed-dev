...
function PaletteInspector({
  ...
  const palette = designSystem.palettes.find(
    (item) => item.id === recipe.theme.paletteId,
  );
  ...
  return (
    <div className="space-y-6">
      <InspectorGroup title="Global palette" note="Shared registry">
          ...
          {designSystem.palettes.map((item) => {
            ...
            return (
              <button
                ...
                onClick={() =>
                  update((next) => {
                    next.theme.paletteId = item.id;
                  })
                }
                className={`w-full rounded-[12px] border p-3 text-left transition ${
                  active
                    ? "border-cyan-300/24 bg-cyan-400/[0.055]"
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <div className="flex -space-x-1">
                    {[
                      item.roles.primary,
                      item.roles.secondary,
                      item.roles.tertiary,
                      item.roles.quaternary,
                    ].map((rgb, index) => (
                      <span
                        key={`${item.id}-${index}`}
                  ...
                  {active ? (
                    <span className="font-mono text-[7px] uppercase text-cyan-200">
                      linked
                    </span>
                  ) : null}
                </div>
                ...
                <span className="mt-1 block text-[8px] leading-4 text-slate-600">
                  {item.description}
                </span>
              </button>
            );
        ...
        <button
          type="button"
          onClick={() => onOpenParameters("palettes")}
          className="flex w-full items-center justify-between rounded-[10px] border border-cyan-300/14 bg-cyan-400/[0.03] px-3 py-2.5 text-[9px] text-cyan-100/75"
        >
      ...
      <InspectorGroup
        ...
        note={palette?.label ?? "Custom colors"}
      >
        <div className="grid grid-cols-2 gap-2">
          {Object.entries(
            resolvedRecipe.theme.resolvedPalette ?? {
              primary: resolvedRecipe.theme.accentRgb,
            },
          ).map(([role, rgb]) => (
            <div
              key={role}
              className="rounded-[10px] border border-white/[0.07] bg-black/20 p-2.5"
            >
              <div
                className="h-7 rounded-[7px]"
                style={{ background: `rgb(${rgb})` }}
              />
              <div className="mt-2 font-mono text-[7px] uppercase text-slate-600">
                {role}
              </div>
            </div>
          ))}
        ...
        <ColorField
          ...
          onChange={(value) =>
            update((next) => {
              ...
              next.theme.paletteId = undefined;
            })
          }
        />
        <RangeField
  ...
  return (
    <div className="space-y-6">
      <InspectorGroup title="Global typography" note="Shared registry">
          ...
          {designSystem.typography.map((preset) => {
            ...
            return (
              <button
                ...
                onClick={() =>
                  update((next) => {
                    next.theme.typographyId = preset.id;
                  })
                }
                className={`w-full rounded-[12px] border p-3 text-left transition ${
                  active
              ...
              >
                <strong
                  className={`block text-[13px] text-white ${
                    preset.displayFont === "serif"
                      ? "font-serif"
                      : preset.displayFont === "mono"
                        ? "font-mono"
                        : "font-sans"
                  } ${preset.titleCase === "uppercase" ? "uppercase" : ""}`}
                >
                  {preset.label}
                </strong>
                <span className="mt-1 block text-[8px] leading-4 text-slate-600">
                ...
                <span className="mt-2 block font-mono text-[7px] uppercase text-slate-700">
                  {preset.displayFont} · {preset.bodyFont} · {preset.eyebrowStyle}
                </span>
              </button>
            );
          })}
        ...
        <button
          type="button"
          onClick={() => onOpenParameters("typography")}
          className="flex w-full items-center justify-between rounded-[10px] border border-cyan-300/14 bg-cyan-400/[0.03] px-3 py-2.5 text-[9px] text-cyan-100/75"
        >
...
function IconographyInspector({
  ...
  return (
    <div className="space-y-6">
      ...
      <InspectorGroup
        title="Child symbols"
        note={`${children.length} destinations`}
      >
        <div className="space-y-1.5">
          {children.map((child) => (
            <button
              ...
              className="flex w-full items-center justify-between rounded-[10px] border border-white/[0.07] bg-white/[0.018] px-3 py-2.5 text-left hover:bg-white/[0.035]"
            >
              <span className="truncate text-[9px] text-slate-400">
                {child.label}
              </span>
              <span className="flex items-center gap-2 font-mono text-[8px] text-slate-700">
                {child.icon}
                <ArrowRight size={10} />
              </span>
            </button>
  ...
  return (
    <div className="space-y-6">
      <InspectorGroup title="Topology" note={`${children.length} direct children`}>
        ...
        <button
          type="button"
          onClick={() => onOpenParameters("children")}
          className="flex w-full items-center justify-between rounded-[10px] border border-cyan-300/14 bg-cyan-400/[0.03] px-3 py-2.5 text-[9px] text-cyan-100/75"
        >
          Open children matrix <ArrowRight size={12} />
        </button>
      </InspectorGroup>
        ...
        <div className="space-y-1.5">
          {children.map((child, index) => (
            <button
              ...
              type="button"
              onClick={() => onSelect(child.selection)}
              className="flex w-full items-center gap-3 rounded-[10px] border border-white/[0.07] bg-white/[0.018] px-3 py-2.5 text-left hover:bg-white/[0.035]"
            >
  ...
  return (
    <div className="space-y-6">
      <InspectorGroup title="Widget shell" note="Shared section grammar">
        ...
        <button
          type="button"
          onClick={() => onOpenParameters("widgets")}
          className="flex w-full items-center justify-between rounded-[10px] border border-cyan-300/14 bg-cyan-400/[0.03] px-3 py-2.5 text-[9px] text-cyan-100/75"
        >
          Open widgets matrix <ArrowRight size={12} />
        </button>
      </InspectorGroup>
      <InspectorGroup
        title="Widget inventory"
        note={`${recipe.sections.length} sections`}
      >
        <div className="space-y-1.5">
