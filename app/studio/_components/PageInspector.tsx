...
import { PAGE_PALETTES, STYLE_PRESETS, applyStylePreset } from "@/app/studio/_components/studio-presets";
  ...
  return (
    <div className="space-y-6">
      <InspectorGroup title="Quick style" note="Whole-page presets">
        <PresetGrid presets={STYLE_PRESETS} onApply={(id) => update((next) => applyStylePreset(next, id))} />
      </InspectorGroup>
      ...
      <InspectorGroup title="Atmosphere" note="Background system">
        <PaletteField label="Palette" value={recipe.theme.accentRgb} colors={PAGE_PALETTES} onChange={(value) => update((next) => { next.theme.accentRgb = value; })} />
        <ColorField label="Custom accent" rgb={recipe.theme.accentRgb} onChange={(value) => update((next) => { next.theme.accentRgb = value; })} />
