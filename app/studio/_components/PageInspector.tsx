  ...
  return (
    <div className="space-y-6">
      ...
      <InspectorGroup title="Identity" note="Page override">
        <TextField
          label="Title"
          value={recipe.identity.title}
          onChange={(value) =>
            update((next) => {
              next.identity.title = value;
            })
          }
        />
        <TextField
        ...
        <TextArea
          label="Subtitle"
          value={recipe.identity.subtitle}
          rows={5}
          onChange={(value) =>
            update((next) => {
              next.identity.subtitle = value;
            })
          }
        />
        <IconGridField
      ...
      <InspectorGroup title="Composition" note="Semantic tokens">
        <ChoiceField
          label="Content width"
          value={recipe.theme.contentWidth ?? "standard"}
          options={["focused", "standard", "wide"]}
          columns={3}
          onChange={(value) =>
            update((next) => {
              next.theme.contentWidth = value as PageContentWidth;
            })
          }
        />
        <ChoiceField
      ...
      <InspectorGroup title="Surfaces" note="Depth and emphasis">
        ...
        <ChoiceField
          label="Border"
          value={recipe.theme.borderStrength ?? "standard"}
          options={["subtle", "standard", "strong"]}
          columns={3}
          onChange={(value) =>
            update((next) => {
              next.theme.borderStrength = value as PageBorderStrength;
            })
          }
        />
        <ChoiceField
          label="Shadow"
          value={recipe.theme.shadow ?? "soft"}
          options={["none", "soft", "dramatic"]}
          columns={3}
          onChange={(value) =>
            update((next) => {
              next.theme.shadow = value as PageShadow;
            })
          }
        />
        <RangeField
      ...
      <InspectorGroup title="Atmosphere" note="Background system">
        <PaletteField
          label="Palette"
          value={recipe.theme.accentRgb}
          colors={PAGE_PALETTES}
          onChange={(value) =>
            update((next) => {
        ...
        <ChoiceField
          label="Motion"
          value={recipe.theme.motion}
          options={["off", "subtle", "expressive"]}
          columns={3}
          onChange={(value) =>
            update((next) => {
              next.theme.motion = value as PageMotion;
            })
          }
        />
        <RangeField
      ...
      {recipe.organization.kind === "multiple-lenses" ? (
        <InspectorGroup title="Lens layout" note="Navigation topology">
          <TextField
            ...
            value={recipe.organization.eyebrow}
            onChange={(value) =>
              update((next) => {
                if (next.organization.kind === "multiple-lenses") {
                  next.organization.eyebrow = value;
                }
              })
            }
          />
          <TextField
            label="Section title"
            value={recipe.organization.title}
            onChange={(value) =>
              update((next) => {
                if (next.organization.kind === "multiple-lenses") {
                  next.organization.title = value;
                }
              })
            }
          />
          <TextArea
            ...
            value={recipe.organization.description}
            rows={4}
            onChange={(value) =>
              update((next) => {
                if (next.organization.kind === "multiple-lenses") {
                  next.organization.description = value;
                }
              })
            }
          />
