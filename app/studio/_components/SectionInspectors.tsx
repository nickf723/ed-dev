...
import type { CaseStudyColumn, CaseStudySection, ModelChoice, ModelGuideSection, PageRecipe, PageSection } from "@/lib/page-system/schema";
    ...
    <div className="space-y-6">
      <InspectorGroup title="Supporting section" note={section.type}>
        ...
        {section.type === "case-study" ? (
          <AddButton label="Add a case-study column" onClick={() => addCaseColumn(section, update, onSelect)} />
        ) : (
          <AddButton label="Add a model choice" onClick={() => addModelChoice(section, update, onSelect)} />
        )}
      </InspectorGroup>
  ...
  return (
    <div className="space-y-6">
      <InspectorGroup title="Case-study column" note={section.title}>
        ...
        <ColorField label="Accent" rgb={column.accentRgb} onChange={(value) => updateCaseColumn(update, section.id, column.id, (next) => { next.accentRgb = value; })} />
      </InspectorGroup>
  ...
  return (
    <div className="space-y-6">
      <InspectorGroup title="Model choice" note={section.title}>
        ...
        <ColorField label="Accent" rgb={choice.accentRgb} onChange={(value) => updateModelChoice(update, section.id, choice.id, (next) => { next.accentRgb = value; })} />
      </InspectorGroup>
