  ...
  return (
    <div className="space-y-6">
      <InspectorGroup title="Lens content" note="Navigation item">
        ...
        <ChoiceField label="Global color role" value={item.colorRole ?? "custom"} options={COLOR_ROLE_OPTIONS} columns={3} onChange={(value) => updateLens(update, item.id, (next) => { next.colorRole = value === "custom" ? undefined : value as DesignAccentRole; })} />
        <ColorField label="Custom accent" rgb={item.accentRgb} onChange={(value) => updateLens(update, item.id, (next) => { next.accentRgb = value; next.colorRole = undefined; })} />
      </InspectorGroup>
  ...
  return (
    <div className="space-y-6">
      <InspectorGroup title="Regime" note="Parallel region">
        ...
        <ChoiceField label="Global color role" value={group.colorRole ?? "custom"} options={COLOR_ROLE_OPTIONS} columns={3} onChange={(value) => updateRegime(update, group.id, (next) => { next.colorRole = value === "custom" ? undefined : value as DesignAccentRole; })} />
        <ColorField label="Custom accent" rgb={group.accentRgb} onChange={(value) => updateRegime(update, group.id, (next) => { next.accentRgb = value; next.colorRole = undefined; })} />
  ...
  return (
    <div className="space-y-6">
      <InspectorGroup title="Field" note={group.label}>
        ...
        <ChoiceField label="Global color role" value={item.colorRole ?? "custom"} options={COLOR_ROLE_OPTIONS} columns={3} onChange={(value) => updateNavigationItem(update, group.id, item.id, (next) => { next.colorRole = value === "custom" ? undefined : value as DesignAccentRole; })} />
        <ColorField label="Custom accent" rgb={item.accentRgb} onChange={(value) => updateNavigationItem(update, group.id, item.id, (next) => { next.accentRgb = value; next.colorRole = undefined; })} />
      </InspectorGroup>
