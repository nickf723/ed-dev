    ...
    <div className="space-y-6">
      <InspectorGroup title="Lens content" note="Navigation item">
        <TextField
          ...
          value={item.label}
          onChange={(value) =>
            updateLens(update, item.id, (next) => {
              next.label = value;
            })
          }
        />
        ...
        <ChoiceField
          ...
          onChange={(value) =>
            updateLens(update, item.id, (next) => {
              next.status = value as "active" | "planned";
            })
          }
        />
        <TextField
          ...
          value={item.href ?? ""}
          placeholder="Derived from curriculum when blank"
          onChange={(value) =>
            updateLens(update, item.id, (next) => {
              next.href = value || undefined;
            })
          }
        />
        <ChoiceField
          label="Diagram"
          value={item.visual}
          options={["timeline", "map", "network"]}
          columns={3}
          onChange={(value) =>
            updateLens(update, item.id, (next) => {
              next.visual = value as LensItem["visual"];
            })
          }
        />
        <IconGridField
      ...
      />
      <ItemActions
        onDuplicate={() => duplicateLens(recipe, item, update, onSelect)}
        onDelete={() => deleteLens(recipe, item.id, update, onSelect)}
        deleteDisabled={items.length <= 1}
      />
    </div>
  );
}

export function RegimeInspector({
  recipe,
  group,
  update,
  onSelect,
    ...
    <div className="space-y-6">
      <InspectorGroup title="Regime" note="Parallel region">
        <TextField
          ...
          value={group.label}
          onChange={(value) =>
            updateRegime(update, group.id, (next) => {
              next.label = value;
            })
          }
        />
        ...
        <ChoiceField
          ...
          value={group.visual}
          options={["classical", "modern", "neutral"]}
          columns={3}
          onChange={(value) =>
            updateRegime(update, group.id, (next) => {
              next.visual = value as RegimeGroup["visual"];
            })
          }
        />
        <ColorField
        ...
        />
        <AddButton
          label="Add a field to this regime"
          onClick={() => addField(group, update, onSelect)}
        />
      </InspectorGroup>
      <OrderControls
        index={index}
      ...
      />
      <ItemActions
        onDuplicate={() => duplicateRegime(recipe, group, update, onSelect)}
        onDelete={() => deleteRegime(recipe, group.id, update, onSelect)}
...
export function NavigationItemInspector({
  group,
  item,
  update,
  onSelect,
    ...
    <div className="space-y-6">
      <InspectorGroup title="Field" note={group.label}>
        <TextField
          ...
          value={item.label}
          onChange={(value) =>
            updateNavigationItem(update, group.id, item.id, (next) => {
              next.label = value;
            })
          }
        />
      ...
      <OrderControls
        ...
        move={(direction) =>
          update((next) => {
            ...
            const target = next.organization.groups.find(
      ...
      />
      <ItemActions
        onDuplicate={() => duplicateField(group, item, update, onSelect)}
        onDelete={() => deleteField(group, item.id, update, onSelect)}
        deleteDisabled={group.items.length <= 1}
      />
    </div>
  );
}
