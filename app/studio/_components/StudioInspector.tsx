  ...
  return (
    <aside className="flex min-h-0 min-w-0 flex-col overflow-hidden border-l border-white/[0.08] bg-[#0c0f16]">
      <div className="flex h-14 shrink-0 items-center justify-between border-b border-white/[0.08] px-4">
        <div>
          <div className="text-[10px] font-semibold uppercase tracking-[0.13em] text-slate-300">
            Inspector
          </div>
          <div className="mt-0.5 text-[9px] text-slate-600">
            {selectionTitle(recipe, selection)}
          </div>
        </div>
      ...
      <div className="border-t border-white/[0.08] p-3">
        ...
        >
          {saveMessage ||
            "Ctrl+S saves the current recipe. Ctrl+Z undoes the last editor change."}
        </div>
      </div>
    </aside>
  );
...
function Inspector({
  ...
  if (
    ...
    const item = recipe.organization.items.find(
  ...
  if (
    ...
    const group = recipe.organization.groups.find(
      (candidate) => candidate.id === selection.id,
    );
    return group ? (
      <RegimeInspector
  ...
  if (selection.kind === "model-choice") {
    ...
    const choice =
      section?.type === "model-guide"
        ? section.choices.find(
            (candidate) => candidate.id === selection.id,
          )
        : undefined;
