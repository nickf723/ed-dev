...
import DesignGuideInspector from "@/app/studio/_components/DesignGuideInspector";
import {
  LensInspector,
  ...
  return (
    <aside className="flex min-h-0 min-w-0 flex-col overflow-hidden border-l border-white/[0.08] bg-[#0c0f16]">
      ...
      <div className="border-t border-white/[0.08] p-3">
        <div className={`whitespace-pre-wrap rounded-[12px] border px-3 py-2.5 text-[9px] leading-4 ${saveState === "error" ? "border-red-300/15 bg-red-400/[0.035] text-red-200/80" : saveState === "saved" ? "border-emerald-300/15 bg-emerald-400/[0.035] text-emerald-200/75" : "border-white/[0.06] bg-black/20 text-slate-600"}`}>
          {saveMessage || "Ctrl+S saves every changed recipe and global registry file."}
        </div>
      </div>
    </aside>
  );
...
function Inspector({
  ...
  if (selection.kind === "lens" && recipe.organization.kind === "multiple-lenses") {
    ...
    return item ? <LensInspector recipe={recipe} item={item} update={update} onSelect={onSelect} /> : <MissingSelection />;
  }
  if (selection.kind === "regime" && recipe.organization.kind === "split-regimes") {
    ...
    return group ? <RegimeInspector recipe={recipe} group={group} update={update} onSelect={onSelect} /> : <MissingSelection />;
  }
  if (selection.kind === "navigation-item" && recipe.organization.kind === "split-regimes") {
    ...
    return group && item ? <NavigationItemInspector group={group} item={item} update={update} onSelect={onSelect} /> : <MissingSelection />;
  }
  if (selection.kind === "section") {
    const section = recipe.sections.find((candidate) => candidate.id === selection.id);
    return section ? <SectionInspector recipe={recipe} section={section} update={update} onSelect={onSelect} /> : <MissingSelection />;
  }
  if (selection.kind === "case-column") {
    const section = recipe.sections.find((candidate) => candidate.id === selection.sectionId);
    const column = section?.type === "case-study" ? section.columns.find((candidate) => candidate.id === selection.id) : undefined;
    return section?.type === "case-study" && column ? <CaseColumnInspector section={section} column={column} update={update} onSelect={onSelect} /> : <MissingSelection />;
  }
  if (selection.kind === "model-choice") {
    const section = recipe.sections.find((candidate) => candidate.id === selection.sectionId);
    const choice = section?.type === "model-guide" ? section.choices.find((candidate) => candidate.id === selection.id) : undefined;
    return section?.type === "model-guide" && choice ? <ModelChoiceInspector section={section} choice={choice} update={update} onSelect={onSelect} /> : <MissingSelection />;
  }
  return <MissingSelection />;
}

function MissingSelection() {
  return <div className="rounded-[14px] border border-white/[0.07] bg-black/20 p-4 text-[10px] leading-5 text-slate-500">That region no longer exists. Select another region in the canvas or structure tree.</div>;
}
