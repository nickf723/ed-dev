...
const ORBITAL_SCENES = [
  {
    ...
    description: "No node and no preferred direction: probability is concentrated around the nucleus.",
    accentRgb: "34, 211, 238",
  },
  {
    id: "2s",
    label: "2s · radial node",
    description: "Two probability regions are separated by a radius where the wavefunction is zero.",
    accentRgb: "52, 211, 153",
  },
  {
    id: "2px",
    label: "2pₓ · angular node",
    description: "Opposite phase lobes lie along one axis and are separated by a nodal plane.",
    accentRgb: "192, 132, 252",
  },
  {
    id: "2py",
    label: "2pᵧ · orientation",
    description: "The same p-state geometry points along a different spatial axis.",
    ...
    <SceneFrame
      ...
      header={
        <DomainPageHeader
          ...
          subtitle="Quantum chemistry replaces miniature planetary orbits with states, amplitudes, and probability distributions. Those states explain electronic structure, periodicity, bonding, spectroscopy, and the geometry of chemical possibility."
          accentRgb="34, 211, 238"
          titleClassName="font-sans text-[clamp(2.7rem,5.4vw,6.2rem)] font-semibold leading-[0.84] tracking-[-0.066em] text-[#f7fbff]"
    ...
    >
      <section className="mt-6">
        <WorldWindow
          ...
          title="An orbital is a structured probability state, not a track around the nucleus."
          description="Sample four hydrogen-like orbitals. Empty regions and sign changes are not missing animation; they are nodes encoded by the wavefunction. Shape, orientation, and energy are properties of the state itself."
      ...
      <section className="mt-12 grid gap-5 xl:grid-cols-[minmax(0,1.08fr)_minmax(380px,0.92fr)]">
        <Surface variant="glass" className="rounded-[32px] p-6 sm:p-8">
          ...
          <h2 className="mt-3 text-[clamp(2rem,4vw,4rem)] font-semibold leading-[0.94] tracking-[-0.054em] text-white">
          ...
          <div className="mt-6 overflow-x-auto rounded-[20px] border border-cyan-100/[0.10] bg-black/[0.24] p-5 text-center text-[18px] text-white sm:p-7 sm:text-[22px]">
            <M display>{String.raw`i\hbar\frac{\partial}{\partial t}\Psi(\mathbf{r},t)=\hat{H}\Psi(\mathbf{r},t)`}</M>
          </div>
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            <Concept label="State" text="The mathematical object containing amplitudes for possible outcomes." rgb="34, 211, 238" />
            <Concept label="Hamiltonian" text="The operator representing the system's total energy and constraints." rgb="192, 132, 252" />
            <Concept label="Probability" text="The squared magnitude of an amplitude predicts outcome frequency." rgb="52, 211, 153" />
          </div>
        </Surface>
        ...
        <Surface variant="ghost" className="rounded-[32px] p-6 sm:p-8">
          ...
          <h2 className="mt-3 text-[clamp(1.9rem,3.5vw,3.3rem)] font-semibold leading-[0.96] tracking-[-0.05em] text-white">
          ...
          <div className="mt-6 rounded-[20px] border border-violet-100/[0.11] bg-black/[0.24] p-6 text-center text-[22px] text-white">
            <M display>{String.raw`\Delta x\,\Delta p\geq\frac{\hbar}{2}`}</M>
          </div>
          <div className="mt-5 rounded-[17px] border border-white/[0.08] bg-white/[0.018] p-4 text-[14px] leading-6 text-slate-300/68">
...
function Concept({ label, text, rgb }: { label: string; text: string; rgb: string }) {
  return (
    <div className="rounded-[17px] border border-white/[0.08] bg-black/[0.18] p-4">
      <div className="font-mono text-[11px] font-semibold uppercase tracking-[0.09em]" style={{ color: `rgba(${rgb},0.74)` }}>
        {label}
      </div>
...
function ModuleCard({
  ...
  return (
    <Surface
      variant="ghost"
      className="flex min-h-[300px] flex-col rounded-[24px] p-5 transition hover:-translate-y-1 hover:bg-black/[0.28]"
      style={{ borderColor: `rgba(${rgb},0.16)` }}
    >
      <span
        className="flex h-12 w-12 items-center justify-center rounded-[15px] border"
        style={{ color: `rgb(${rgb})`, borderColor: `rgba(${rgb},0.28)`, background: `rgba(${rgb},0.055)` }}
      >
        <Icon size={19} />
      </span>
      <h3 className="mt-6 text-[21px] font-semibold tracking-[-0.035em] text-white">{title}</h3>
      ...
      <span className="mt-auto flex items-center gap-2 pt-6 font-mono text-[11px] font-semibold uppercase tracking-[0.09em]" style={{ color: `rgba(${rgb},0.72)` }}>
