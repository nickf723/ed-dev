...
import {
  ...
  type MethaneReactionCoefficients,
} from "../chemistryModel";
...
export default function ReactionBalancer() {
  ...
  const balanced = isMethaneReactionBalanced(coefficients);

  function adjust(key: keyof MethaneReactionCoefficients, amount: number) {
  ...
  return (
    <article
      data-model="methane-conservation-ledger"
      className="overflow-hidden rounded-[24px] border border-amber-100/[0.11] bg-black/[0.25] shadow-[0_24px_85px_rgba(0,0,0,0.22)] backdrop-blur-xl"
    >
      <div className="flex flex-col gap-3 border-b border-white/[0.08] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="text-amber-200/74 flex items-center gap-2 font-mono text-[11px] font-semibold uppercase tracking-[0.12em]">
      ...
      <div className="p-5 sm:p-6">
        ...
        <div
          className={`mt-5 flex items-start gap-3 rounded-[16px] border p-4 text-[14px] leading-6 ${
            balanced
              ? "text-emerald-100/82 border-emerald-300/[0.18] bg-emerald-300/[0.045]"
              : "text-slate-300/72 border-white/[0.08] bg-white/[0.018]"
          }`}
        >
          {balanced ? (
            <CheckCircle2
            ...
            <Scale size={18} className="text-amber-200/66 mt-0.5 shrink-0" />
          )}
          <p>
            {balanced
  ...
  return (
    <div className="flex items-center gap-2.5">
      <div className="grid gap-1">
        ...
        />
        <span
          className={`min-w-8 text-center text-[24px] font-semibold ${value > 1 ? "text-emerald-200" : "text-slate-500"}`}
        >
          {value}
        </span>
        <ControlButton
          label="Decrease coefficient"
          onClick={onDecrease}
          icon={Minus}
        />
      </div>
...
function ControlButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      onClick={onClick}
      className="flex h-8 w-8 items-center justify-center rounded-[8px] border border-white/[0.08] bg-white/[0.025] text-slate-500 transition hover:bg-white/[0.08] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-200/60"
    >
      <Icon size={13} />
    </button>
  );
...
function Operator({ children }: { children: ReactNode }) {
  return (
    <span className="text-white/28 px-1 text-[24px] font-light">
      {children}
    </span>
  );
}
        ...
        {(["C", "H", "O"] as const).map((element) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`font-mono font-semibold ${matches ? "text-emerald-300" : "text-slate-200"}`}
              >
                {counts[element]}
              </span>
            </div>
