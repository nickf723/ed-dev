...
const WEATHER_ENGINE = [
  {
    ...
    detail: "Sun angle, surface type, clouds, water, vegetation, and season create temperature contrasts across the surface and atmosphere.",
    rgb: "250, 204, 21",
  },
  {
    icon: Gauge,
    label: "Pressure gradients",
    detail: "Density and temperature differences help produce pressure fields. Air accelerates when pressure differs across space.",
    rgb: "125, 211, 252",
  },
  {
    icon: Wind,
    label: "Wind & rotation",
    detail: "Pressure-gradient force, Coriolis deflection, friction, and curvature shape the direction and speed of atmospheric flow.",
    rgb: "56, 189, 248",
  },
  {
    icon: CloudRain,
    label: "Moisture & phase change",
    detail: "Evaporation adds water vapor; lifting and cooling can bring air to saturation, forming cloud droplets or ice and eventually precipitation.",
    rgb: "34, 211, 238",
  },
] as const;
...
export default function MeteorologyPage() {
  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden bg-[#030914] text-slate-100 selection:bg-sky-400/25">
      <WindBackground />
      <div className="pointer-events-none fixed inset-0 z-0 bg-[radial-gradient(circle_at_78%_14%,rgba(56,189,248,0.13),transparent_29%),radial-gradient(circle_at_16%_84%,rgba(129,140,248,0.05),transparent_28%),linear-gradient(to_bottom,rgba(3,9,20,0.08),rgba(3,9,20,0.74)_76%,rgba(2,6,14,0.97))]" aria-hidden="true" />
      <div className="pointer-events-none fixed inset-0 z-[1] opacity-[0.09] [background-image:linear-gradient(rgba(125,211,252,0.08)_1px,transparent_1px)] [background-size:100%_52px] [mask-image:linear-gradient(to_bottom,black,transparent_92%)]" aria-hidden="true" />
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1580px] px-4 pb-14 sm:px-6 xl:px-8">
        <div className="sticky top-0 z-30 -mx-4 border-b border-white/[0.06] bg-[#030914]/80 px-4 pb-3 pt-5 shadow-[0_18px_58px_rgba(0,0,0,0.24)] backdrop-blur-2xl sm:-mx-6 sm:px-6 xl:-mx-8 xl:px-8">
          <DomainPageHeader
            ...
            eyebrow="Energy · pressure · wind · moisture · instability"
            eyebrowStyle="rule"
            icon={Wind}
            title={<span>Meteorology</span>}
            subtitle="Meteorology studies the evolving atmosphere as a fluid system. Uneven heating creates pressure and density contrasts, air moves and rotates, moisture changes phase, and those processes organize clouds, fronts, storms, and the weather patterns we observe and forecast."
        ...
        <section className="mt-5 overflow-hidden rounded-[30px] border border-sky-200/[0.10] bg-black/[0.14] backdrop-blur-xl">
          <div className="grid gap-4 border-b border-white/[0.07] px-5 py-5 lg:grid-cols-[minmax(0,1fr)_420px] lg:items-end sm:px-6">
            <div>
              <div className="flex items-center gap-2 font-mono text-[9px] font-semibold uppercase tracking-[0.14em] text-sky-200/58"><ThermometerSun size={12} /> Weather engine</div>
              <h2 className="mt-2 text-[clamp(1.9rem,3.6vw,3.3rem)] font-semibold leading-[0.94] tracking-[-0.05em] text-white">Weather begins with gradients and becomes motion.</h2>
            </div>
            <p className="text-[12px] leading-6 text-slate-400">The upper atmosphere matters, but most everyday weather is organized in the troposphere. The useful chain is energy contrast → pressure field → wind → vertical motion → moisture and phase change.</p>
          </div>
          <div className="grid md:grid-cols-2 xl:grid-cols-4">
            {WEATHER_ENGINE.map((item, index) => {
        ...
        <section className="mt-6 overflow-hidden rounded-[28px] border border-white/[0.08] bg-black/[0.13] backdrop-blur-xl">
          <div className="grid gap-4 border-b border-white/[0.07] px-5 py-5 lg:grid-cols-[minmax(0,1fr)_430px] lg:items-end sm:px-6">
            <div><div className="font-mono text-[9px] font-semibold uppercase tracking-[0.14em] text-indigo-200/50">Organized weather systems · reference</div><h2 className="mt-2 text-[clamp(1.7rem,3vw,2.8rem)] font-semibold tracking-[-0.045em] text-white">Atmospheric processes combine into structures across many scales.</h2></div>
            <p className="text-[11px] leading-5 text-slate-500">A cloud, thunderstorm, cyclone, front, and jet stream are not separate ingredients. They are organized outcomes of the same fluid, thermodynamic, moisture, and rotation processes acting at different scales.</p>
          </div>
