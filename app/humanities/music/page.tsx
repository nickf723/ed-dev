...
import MixingConsoleTopology, { type MixChannel } from "@/app/_page-system/topologies/MixingConsoleTopology";
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1560px] px-4 pb-14 sm:px-6 xl:px-8">
        ...
        <section className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1.15fr)_minmax(330px,0.85fr)]">
          <div className="rounded-[28px] border border-rose-200/[0.11] bg-black/[0.11] p-5 shadow-[0_28px_95px_rgba(0,0,0,0.20)] backdrop-blur-xl sm:p-6">
            <div className="font-mono text-[9px] font-semibold uppercase tracking-[0.15em] text-rose-200/70">The central distinction</div>
            <h2 className="mt-2 max-w-4xl text-[clamp(1.8rem,3.2vw,2.9rem)] font-semibold tracking-[-0.05em] text-white">
              A composition, a performance, and a recording can refer to the “same song” without being the same object.
            </h2>
            <p className="mt-3 max-w-3xl text-[11px] leading-6 text-slate-400">
              Music theory studies structural relationships. Performance adds interpretation and embodied technique. Recording fixes one sounding event into a reproducible artifact. History and culture explain why those structures and practices mean what they do.
            </p>
          </div>

          <div className="grid grid-cols-3 gap-2 rounded-[28px] border border-white/[0.08] bg-black/[0.11] p-3 backdrop-blur-xl">
            <ObjectCard icon={SlidersVertical} label="Composition" text="A structured musical design that can survive many performances." rgb="244, 114, 182" />
            <ObjectCard icon={Mic2} label="Performance" text="A situated realization shaped by people, instruments, and interpretation." rgb="251, 146, 60" />
            <ObjectCard icon={Radio} label="Recording" text="A captured performance shaped again by microphones, editing, mixing, and media." rgb="34, 211, 238" />
          </div>
        </section>
...
function ObjectCard({ icon: Icon, label, text, rgb }: { icon: LucideIcon; label: string; text: string; rgb: string }) {
  return (
...
function ListeningLayer({ label, question, text, rgb }: { label: string; question: string; text: string; rgb: string }) {
  return (
    <div className="relative overflow-hidden rounded-[24px] border bg-black/[0.11] p-5 backdrop-blur-xl" style={{ borderColor: `rgba(${rgb},0.11)` }}>
      <div className="absolute -right-10 -top-10 h-28 w-28 rounded-full blur-[40px]" style={{ background: `rgba(${rgb},0.07)` }} />
      <div className="relative z-10 font-mono text-[8px] font-semibold uppercase tracking-[0.13em]" style={{ color: `rgba(${rgb},0.65)` }}>{label}</div>
      <h3 className="relative z-10 mt-2 text-[17px] font-semibold text-white">{question}</h3>
      <p className="relative z-10 mt-2 text-[9px] leading-5 text-slate-600">{text}</p>
    </div>
  );
}
