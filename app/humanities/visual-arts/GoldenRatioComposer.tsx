  ...
  return (
    <div className="overflow-hidden rounded-[24px] border border-white/[0.08] bg-[#0c0908]/80 shadow-[0_24px_85px_rgba(0,0,0,0.24)]">
      <div className="grid lg:grid-cols-[260px_minmax(0,1fr)]">
        <aside className="border-b border-white/[0.07] p-4 sm:p-5 lg:border-b-0 lg:border-r">
          <div className="flex items-center gap-2 font-mono text-[10px] font-semibold uppercase tracking-[0.09em] text-orange-200/65">
            <LayoutTemplate size={13} /> Composition guides
          </div>
          <h3 className="mt-2 text-[19px] font-semibold text-white">
            Compare frameworks, not formulas.
          </h3>
        ...
        <div className="relative min-h-[400px] overflow-hidden bg-[radial-gradient(circle_at_25%_18%,rgba(244,63,94,0.18),transparent_35%),radial-gradient(circle_at_74%_72%,rgba(59,130,246,0.18),transparent_36%),linear-gradient(145deg,#171012,#09090c)]">
          ...
          <div className="bg-blue-400/28 absolute bottom-[15%] right-[8%] h-[16%] w-[58%] rotate-[-10deg] rounded-full blur-[1px]" />
          <div className="bg-amber-300/12 absolute left-[40%] top-[15%] h-[54%] w-[12%] rotate-[18deg]" />
          ...
          {overlay === "thirds" ? (
            <div className="pointer-events-none absolute inset-0 grid grid-cols-3 grid-rows-3">
              {Array.from({ length: 9 }, (_, index) => (
                <div key={index} className="border border-blue-200/[0.19]" />
              ))}
            </div>
          ) : null}
          ...
          {overlay === "golden" ? (
            ...
            >
              ...
              />
              <path
                d="M0 100 Q0 0 61.8 0 Q100 0 100 38.2 Q100 61.8 76.4 61.8 Q61.8 61.8 61.8 47.2"
                fill="none"
          ...
          <div className="absolute inset-x-4 bottom-4 rounded-[13px] border border-white/[0.08] bg-black/35 px-4 py-3 backdrop-blur-md">
            <div className="flex items-center justify-between gap-4 font-mono text-[10px] uppercase tracking-[0.06em] text-white/55">
              <span>click the focal marker to move it</span>
              <span>
                x {focalX} · y {focalY}
              </span>
            </div>
...
function GuideButton({
  ...
  return (
    <button
      ...
      style={{
        borderColor: `rgba(${rgb},${active ? 0.3 : 0.09})`,
        background: active ? `rgba(${rgb},0.055)` : "rgba(255,255,255,0.012)",
        color: active ? `rgb(${rgb})` : "rgb(168,162,158)",
      }}
    >
      <Icon size={15} />
