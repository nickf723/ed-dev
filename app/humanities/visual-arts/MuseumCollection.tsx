...
import { ExternalLink, Filter, ImageIcon, LoaderCircle, RotateCcw, Search, ShieldCheck, Sparkles, TriangleAlert, X } from "lucide-react";
...
export default function MuseumCollection() {
  ...
  async function search(event?: FormEvent, override?: string) {
    ...
    try {
      ...
      if (!response.ok) {
        applyFallback(payload.error || "Unable to search the museum collection.", payload.provenance.state);
        return;
      }
      ...
      if (requestId !== requestSequence.current) return;
      applyFallback(reason instanceof Error ? reason.message : "Unable to search the museum collection.", "failed");
    } finally {
  ...
  return (
    <section aria-busy={loading} className="overflow-hidden rounded-[30px] border border-white/[0.08] bg-black/[0.11] shadow-[0_32px_110px_rgba(0,0,0,0.25)] backdrop-blur-xl">
      ...
      <div className="border-b border-white/[0.07] px-5 py-3 sm:px-6">
        <div className="flex flex-wrap items-center gap-2">
          <span className="mr-1 font-mono text-[10px] uppercase tracking-[0.07em] text-stone-500">Try</span>
        ...
        {facets.length > 0 ? (
          <div className="mt-3 flex flex-wrap items-end gap-2 border-t border-white/[0.05] pt-3">
            <span className="mr-1 flex h-10 items-center gap-2 font-mono text-[10px] font-semibold uppercase tracking-[0.08em] text-stone-600"><Filter size={12} /> Refine sample</span>
            {facets.map((facet) => (
              <MuseumFacetSelect
                key={facet.id}
                facet={facet}
            ...
            {result.activeFilterCount > 0 ? (
              <button type="button" onClick={clearFacets} className="flex h-10 items-center gap-2 rounded-[12px] border border-white/[0.07] px-3 text-[10px] font-semibold text-stone-500 transition hover:border-white/20 hover:text-white"><RotateCcw size={11} /> Clear filters</button>
            ) : null}
          </div>
        ) : null}
      </div>
      ...
      <div className="relative p-4 sm:p-5">
        ...
        {error ? <div role="alert" className="mb-4 rounded-[14px] border border-red-300/[0.12] bg-red-400/[0.035] px-4 py-3 text-[11px] leading-5 text-red-100/70">{error} The curated teaching wall is active instead.</div> : null}
        {!result.records.length && !loading ? (
          <div className="rounded-[20px] border border-dashed border-white/[0.08] p-10 text-center">
            ...
            <p className="mt-3 text-[12px] text-stone-500">{records.length ? "No objects remain under those filters." : "No image-bearing objects matched that provider search."}</p>
            <button type="button" onClick={records.length ? clearFacets : reset} className="mx-auto mt-4 flex items-center gap-2 rounded-full border border-orange-100/[0.12] px-4 py-2 text-[10px] font-semibold text-orange-100/65 transition hover:border-orange-100/30 hover:text-white"><RotateCcw size={11} /> {records.length ? "Clear sample filters" : "Return to the curated wall"}</button>
          </div>
        ) : <GalleryWallTopology records={result.records} selectedId={selected?.id} onSelect={setSelected} accentRgb="251, 146, 60" />}
      </div>
...
function curated(id: string, title: string, creator: string, year: string, medium: string, tags: string[], description: string): CollectionMediaRecord {
  return {
    ...
    sources: [{ label: "Education Station curated teaching record", kind: "curated", scope: "Interpretive starter description" }],
  };
}
...
function buildMuseumFacets(records: readonly CollectionMediaRecord[]): CollectionFacetDefinition<CollectionMediaRecord>[] {
  const candidates: Array<{
    ...
    },
    {
      id: "medium",
      label: "Medium",
      value: (record) => fact(record, "medium") ?? fact(record, "objectName") ?? record.subtitle,
    },
    {
      id: "rights",
      label: "Image rights",
  ...
  return candidates.flatMap((candidate) => {
    const values = Array.from(new Set(records.map(candidate.value).filter((value): value is string => Boolean(value)))).sort((left, right) => left.localeCompare(right));
...
function MuseumFacetSelect({
  ...
  return (
    <label className="grid min-w-[150px] max-w-[260px] flex-1 gap-1">
      ...
      <select value={value} onChange={(event) => onChange(event.target.value)} className="h-10 min-w-0 rounded-[12px] border border-white/[0.08] bg-[#110d0b] px-3 text-[10px] text-stone-400 outline-none transition focus:border-orange-200/25">
        <option value="">Any {facet.label.toLocaleLowerCase()}</option>
        {facet.options.map((option) => (
          <option key={option.id} value={option.id} disabled={counts[option.id] === 0 && value !== option.id}>{option.label} ({counts[option.id] ?? 0})</option>
        ))}
      </select>
    </label>
  );
    ...
    <div className="mb-4 grid gap-3 rounded-[15px] border border-white/[0.07] bg-white/[0.012] px-4 py-3 md:grid-cols-[minmax(0,1fr)_auto] md:items-center" role="status">
      <div>
        <div className="flex flex-wrap items-center gap-2 font-mono text-[10px] font-semibold uppercase tracking-[0.08em] text-stone-500">
          <Icon size={12} className={caution ? "text-amber-200/58" : "text-emerald-200/58"} />
          {stateLabel(metadata.provenance.state)}
          <span className="text-stone-700">·</span>
          {source?.url ? <a href={source.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 normal-case tracking-normal text-orange-100/55 transition hover:text-white">{source.label}<ExternalLink size={10} /></a> : <span className="normal-case tracking-normal text-stone-600">{source?.label ?? metadata.source}</span>}
        </div>
        {metadata.provenance.note ? <p className="mt-1.5 text-[10px] leading-5 text-stone-600">{metadata.provenance.note}</p> : null}
      </div>
      <div className="font-mono text-[10px] uppercase tracking-[0.06em] text-stone-600 md:text-right">
        ...
        <div className="mt-1 text-stone-700">{total.toLocaleString("en-US")} {metadata.provenance.state === "curated" || metadata.provenance.state === "fallback" ? "teaching records" : "provider matches"}</div>
        {metadata.provenance.retrievedAt ? <div className="mt-1 text-[9px] text-stone-800">Retrieved {metadata.provenance.retrievedAt.replace("T", " ").slice(0, 16)} UTC</div> : null}
      </div>
    </div>
  );
}
