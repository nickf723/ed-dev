  ...
  return (
    <section className="overflow-hidden rounded-[30px] border border-white/[0.08] bg-black/[0.11] shadow-[0_32px_110px_rgba(0,0,0,0.25)] backdrop-blur-xl">
      <div className="grid gap-5 border-b border-white/[0.07] p-5 xl:grid-cols-[minmax(0,1fr)_520px] xl:items-end sm:p-6">
        <div>
          <div className="flex items-center gap-2 font-mono text-[9px] font-semibold uppercase tracking-[0.15em] text-orange-200/70"><ImageIcon size={12} /> Museum collection</div>
          ...
          <p className="mt-3 max-w-3xl text-[10px] leading-5 text-stone-500">Search museum records, then inspect medium, maker, date, culture, dimensions, department, and collection context. A work of art is simultaneously an image, a physical object, a made thing, and a historical artifact.</p>
        </div>
        <form onSubmit={(event) => void search(event)} className="relative">
          ...
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search the museum collection…" className="h-12 w-full rounded-[15px] border border-white/[0.08] bg-black/[0.26] pl-11 pr-12 text-[11px] text-white outline-none placeholder:text-stone-700 focus:border-orange-300/[0.22]" />
          {query ? <button type="button" onClick={reset} aria-label="Clear museum search" className="absolute right-3 top-1/2 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-full text-stone-700 hover:bg-white/[0.04] hover:text-white"><X size={12} /></button> : null}
        </form>
      </div>

      <div className="border-b border-white/[0.07] px-5 py-3 sm:px-6">
        <div className="flex flex-wrap items-center gap-2">
          <span className="mr-1 font-mono text-[7px] uppercase tracking-[0.1em] text-stone-700">Try</span>
          {SUGGESTIONS.map((suggestion) => <button key={suggestion} type="button" onClick={() => void search(undefined, suggestion)} className="rounded-full border border-white/[0.06] bg-white/[0.015] px-2.5 py-1.5 text-[8px] text-stone-600 transition hover:text-orange-200">{suggestion}</button>)}
          <span className="ml-auto font-mono text-[7px] uppercase tracking-[0.08em] text-stone-800">{source} · {mediums.length || 1} media</span>
        </div>
      </div>

      <div className="relative p-4 sm:p-5">
        {loading ? <div className="absolute inset-0 z-20 flex items-center justify-center rounded-[20px] bg-black/[0.62] backdrop-blur-sm"><span className="flex items-center gap-2 font-mono text-[9px] uppercase tracking-[0.11em] text-orange-100/70"><LoaderCircle size={14} className="animate-spin" /> opening storage</span></div> : null}
        {error ? <div className="mb-4 rounded-[14px] border border-red-300/[0.12] bg-red-400/[0.035] px-4 py-3 text-[9px] text-red-100/65">{error} The starter wall remains available.</div> : null}
        {!records.length && !loading ? <div className="rounded-[20px] border border-dashed border-white/[0.08] p-10 text-center"><Sparkles size={18} className="mx-auto text-stone-700" /><p className="mt-3 text-[10px] text-stone-600">No image-bearing objects matched that search.</p></div> : <GalleryWallTopology records={records} selectedId={selected?.id} onSelect={setSelected} accentRgb="251, 146, 60" />}
      </div>
