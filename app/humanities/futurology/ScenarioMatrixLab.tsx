...
export default function ScenarioMatrixLab() {
  ...
  return (
    <Surface variant="glass" className="overflow-hidden rounded-[30px] border-cyan-100/[0.12]" style={{ background: "rgba(7,16,25,0.21)" }}>
      <div className="grid border-b border-white/[0.07] lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="p-5 sm:p-6">
          <div className="flex items-center gap-2 font-mono text-[11px] font-semibold uppercase tracking-[0.09em] text-cyan-200/62"><Compass size={14} /> Scenario planning · fictional case</div>
          ...
          <p className="mt-3 max-w-3xl text-[13px] leading-6 text-slate-400/78">Imagine a fictional metropolitan region planning for 2045. Two critical uncertainties are held apart: future resource pressure and future coordination capacity. The matrix explores combinations of those uncertainties. It does not predict which quadrant will occur.</p>
        </div>
        <div className="border-t border-white/[0.07] bg-black/[0.06] p-5 backdrop-blur-[10px] lg:border-l lg:border-t-0">
          <span className="font-mono text-[10px] uppercase tracking-[0.07em] text-slate-600">Selected scenario</span>
          <strong className="mt-2 block text-[18px]" style={{ color: `rgba(${scenario.rgb},0.88)` }}>{scenario.label}</strong>
          <p className="mt-2 text-[11px] leading-5 text-slate-500">A scenario is useful when its assumptions are visible enough to question and its consequences are coherent enough to stress-test decisions.</p>
        </div>
      </div>

      <div className="grid gap-5 p-4 sm:p-5 xl:grid-cols-[minmax(360px,0.95fr)_minmax(0,1.2fr)_285px]">
        <div>
          <div className="mb-3 font-mono text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-600">Critical uncertainties</div>
          <div className="relative aspect-square overflow-hidden border border-white/[0.07] bg-[#08111c]/48 p-10 backdrop-blur-[8px]">
            ...
            <span className="absolute left-1/2 top-3 -translate-x-1/2 font-mono text-[9px] uppercase tracking-[0.06em] text-red-200/45">higher resource pressure</span>
            <span className="absolute bottom-3 left-1/2 -translate-x-1/2 font-mono text-[9px] uppercase tracking-[0.06em] text-cyan-200/42">lower resource pressure</span>
            <span className="absolute left-2 top-1/2 -translate-y-1/2 -rotate-90 font-mono text-[9px] uppercase tracking-[0.06em] text-amber-200/38">fragmented</span>
            <span className="absolute right-1 top-1/2 -translate-y-1/2 rotate-90 font-mono text-[9px] uppercase tracking-[0.06em] text-violet-200/42">coordinated</span>
        ...
        <div className="border border-white/[0.07] bg-black/[0.04] p-4 sm:p-5">
          <span className="font-mono text-[10px] uppercase tracking-[0.07em]" style={{ color: `rgba(${scenario.rgb},0.62)` }}>{scenario.pressure} pressure · {scenario.coordination} coordination</span>
          <h4 className="mt-2 text-[23px] font-semibold tracking-[-0.035em] text-white">{scenario.label}</h4>
          <p className="mt-3 text-[12px] leading-6 text-slate-400/78">{scenario.summary}</p>
        ...
        <div>
          <div className="font-mono text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-600">Robust across several futures</div>
          <div className="mt-3 divide-y divide-white/[0.06] border-y border-white/[0.07]">
            {ROBUST_MOVES.map((move, index) => <div key={move} className="grid grid-cols-[32px_minmax(0,1fr)] gap-2 py-3"><span className="font-mono text-[10px] text-cyan-200/42">0{index + 1}</span><p className="text-[11px] leading-5 text-slate-500">{move}</p></div>)}
          </div>
          <div className="mt-4 border-l-2 border-violet-300/24 bg-violet-300/[0.025] px-3 py-3">
            <span className="font-mono text-[10px] uppercase tracking-[0.07em] text-violet-200/50">Method boundary</span>
            <p className="mt-2 text-[11px] leading-5 text-slate-500">Scenario planning is not a substitute for forecasting when reliable quantitative prediction is possible. It is especially useful when several important uncertainties interact and a decision should survive more than one plausible future.</p>
          </div>
        </div>
      </div>
    </Surface>
...
function Quadrant({ scenario, selected, onClick }: { scenario: Scenario; selected: boolean; onClick: () => void }) {
  return <button type="button" onClick={onClick} className="relative flex min-h-0 flex-col justify-end border p-3 text-left transition" style={{ borderColor: selected ? `rgba(${scenario.rgb},0.42)` : "rgba(255,255,255,0.055)", background: selected ? `rgba(${scenario.rgb},0.085)` : `rgba(${scenario.rgb},0.022)` }}><span className="font-mono text-[8px] uppercase tracking-[0.06em]" style={{ color: `rgba(${scenario.rgb},0.55)` }}>{scenario.pressure} / {scenario.coordination}</span><strong className="mt-1 text-[11px] leading-4 text-white/78">{scenario.label}</strong>{selected ? <span className="absolute right-2 top-2 h-2 w-2 rounded-full" style={{ background: `rgb(${scenario.rgb})`, boxShadow: `0 0 10px rgba(${scenario.rgb},0.42)` }} /> : null}</button>;
}

function EvidenceList({ icon: Icon, title, items, rgb }: { icon: typeof Layers3; title: string; items: readonly string[]; rgb: string }) {
  return <div className="border border-white/[0.06] bg-black/[0.035] p-3"><div className="flex items-center gap-2 font-mono text-[9px] uppercase tracking-[0.06em]" style={{ color: `rgba(${rgb},0.58)` }}><Icon size={11} /> {title}</div><div className="mt-3 space-y-2">{items.map((item) => <div key={item} className="flex gap-2 text-[10px] leading-4 text-slate-500"><span style={{ color: `rgba(${rgb},0.45)` }}>•</span><span>{item}</span></div>)}</div></div>;
}
