...
// --- PROGRESSION DICTIONARIES ---
export const PROGRESSIONS = {
    'Pop Punk': ['I', 'V', 'vi', 'IV'],
    'Jazz Turnaround': ['ii', 'V', 'I'],
    'Classic Blues': ['I', 'IV', 'V'],
    'Doo-Wop': ['I', 'vi', 'IV', 'V'],
    'Cinematic': ['vi', 'IV', 'I', 'V']
};

// Maps Roman Numerals to their Major Scale distance and chord quality
const ROMAN_TO_INTERVAL = {
    'I': { semitones: 0, formula: 'Major' },
    'ii': { semitones: 2, formula: 'Minor' },
    'iii': { semitones: 4, formula: 'Minor' },
    'IV': { semitones: 5, formula: 'Major' },
    'V': { semitones: 7, formula: 'Major' },
    'vi': { semitones: 9, formula: 'Minor' },
    'vii°': { semitones: 11, formula: 'Diminished' }
};
...
export default function GrandPianoLab() {
    ...
    const [mode, setMode] = useState<'scale' | 'chord' | 'progression'>('scale');
    const [selectedType, setSelectedType] = useState("Major");
    const [selectedProgression, setSelectedProgression] = useState("Pop Punk");
    const [chordStyle, setChordStyle] = useState<'block' | 'arp'>('block');
    ...
    const getTheoryNotes = () => {
        ...
        // If in progression mode, highlight the root major scale to show the sandbox
        const formulaName = mode === 'progression' ? 'Major' : selectedType;
        const formulaDict = mode === 'chord' ? FORMULAS.chords : FORMULAS.scales;
        
        const formula = formulaDict[formulaName as keyof typeof formulaDict] || FORMULAS.scales.Major;
    ...
    useEffect(() => {
        audio.stopAll();
        setActiveKeys([]);
    }, [root, mode, selectedType, selectedProgression]);
    ...
    // Progression & Demo Playback Engine
    const runDemo = async () => {
        ...
        if (mode === 'progression') {
            const prog = PROGRESSIONS[selectedProgression as keyof typeof PROGRESSIONS];
            
            for (const numeral of prog) {
                // Determine absolute notes for the current chord in the progression
                const chordDef = ROMAN_TO_INTERVAL[numeral as keyof typeof ROMAN_TO_INTERVAL];
                const absoluteRoot = NOTES.indexOf(root) + (START_OCTAVE * 12);
                const chordRootAbsolute = absoluteRoot + chordDef.semitones;
                
                // Fallback to Major if a weird quality is requested but missing from DB
                const formula = FORMULAS.chords[chordDef.formula as keyof typeof FORMULAS.chords] || FORMULAS.chords.Major;
                
                const chordSequence = formula.map(interval => {
                    const targetSemitone = chordRootAbsolute + interval;
                    const noteIndex = targetSemitone % 12;
                    const octave = Math.floor(targetSemitone / 12);
                ...
                if (chordStyle === 'block') {
                    chordSequence.forEach(playNote);
                    await new Promise(r => setTimeout(r, beatMs * 2));
                    chordSequence.forEach(stopNote);
                } else {
                    for (const n of chordSequence) {
                        playNote(n);
            ...
            // Original Single Scale / Chord Demo Logic
            ...
            <div className="relative z-50 p-6 flex flex-col gap-6 bg-gradient-to-b from-black/95 to-transparent">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    <TheoryPanel 
                        ...
                        selectedProgression={selectedProgression} setSelectedProgression={setSelectedProgression}
