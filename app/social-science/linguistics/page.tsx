...
import CommunicationLoopTopology, {
  type CommunicationStage,
  type CommunicationStageIcon,
} from "@/app/_page-system/topologies/CommunicationLoopTopology";
...
const META: Record<string, Meta> = {
  ...
  "social.linguistics.psycholinguistics": {
    question:
      "How does a mind produce and understand language in real time?",
    rgb: "167, 139, 250",
    icon: "brain",
    position: { x: 50, y: 13 },
  },
  "social.linguistics.sociolinguistics": {
    question:
      "How does language vary with communities, identity, power, and situation?",
    rgb: "34, 211, 238",
    icon: "users",
    position: { x: 80, y: 31 },
  },
  "social.linguistics.historical": {
    question:
      "How do repeated generations of use change languages over time?",
    rgb: "251, 191, 36",
    icon: "history",
    position: { x: 73, y: 77 },
  },
  "social.linguistics.acquisition": {
    question:
      "How do learners infer a language system from experience and interaction?",
    rgb: "244, 114, 182",
    icon: "sprout",
    position: { x: 27, y: 77 },
  },
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1540px] px-4 pb-14 sm:px-6 xl:px-8">
        ...
        <section className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1.15fr)_minmax(330px,0.85fr)]">
          <div className="rounded-[28px] border border-lime-200/[0.11] bg-black/[0.11] p-5 shadow-[0_28px_95px_rgba(0,0,0,0.20)] backdrop-blur-xl sm:p-6">
            <div className="font-mono text-[11px] font-semibold uppercase tracking-[0.14em] text-lime-200/70">
              The object of study
            </div>
            <h2 className="mt-2 max-w-4xl text-[clamp(1.8rem,3.2vw,2.9rem)] font-semibold tracking-[-0.05em] text-white">
              Language is not only a dictionary plus grammar rules.
            </h2>
            <p className="mt-3 max-w-3xl text-[14px] leading-6 text-slate-400/74">
              A speaker turns intentions into structured signals; a listener
              infers structure and meaning from those signals; both draw on
              learned conventions that vary across groups and change across
              time. Linguistics studies every part of that loop.
            </p>
          </div>
        ...
        <section className="mt-5 overflow-hidden rounded-[28px] border border-white/[0.08] bg-black/[0.12] backdrop-blur-xl">
          <div className="grid gap-4 border-b border-white/[0.07] p-5 lg:grid-cols-[minmax(0,1fr)_420px] lg:items-end sm:p-6">
            <div>
              <div className="flex items-center gap-2 font-mono text-[11px] font-semibold uppercase tracking-[0.13em] text-cyan-200/65">
                <Clock3 size={14} /> Three timescales
              </div>
              <h2 className="mt-2 text-[23px] font-semibold tracking-[-0.04em] text-white">
                The same language system can be studied over milliseconds,
                years, or centuries.
              </h2>
            </div>
            <p className="text-[14px] leading-6 text-slate-400/72">
              Different subfields often differ less in topic than in the
              timescale and population over which they ask the question.
            </p>
          </div>
          <div className="grid md:grid-cols-3">
            <TimeScale
              ...
              title="Processing"
              text="How quickly can a listener recognize sounds, predict words, resolve ambiguity, and assemble sentence structure?"
              rgb="167, 139, 250"
            />
            <TimeScale
              label="Months to decades"
              title="Learning & variation"
              text="How do individuals acquire patterns, adapt to interlocutors, and participate in socially structured variation?"
              rgb="34, 211, 238"
            />
            <TimeScale
              label="Generations to millennia"
              title="Language change"
              text="How do repeated small variations accumulate into new sounds, words, grammatical systems, and language families?"
              rgb="251, 191, 36"
            />
          </div>
        </section>
...
function SignalStep({
  ...
  return (
    <div className="flex min-h-[140px] flex-col items-center justify-center rounded-[18px] border border-white/[0.06] bg-white/[0.012] text-center">
      <Icon size={17} style={{ color: `rgb(${rgb})` }} />
      <span
        className="mt-3 font-mono text-[11px] font-semibold uppercase tracking-[0.09em]"
        style={{ color: `rgba(${rgb},0.62)` }}
      >
        {label}
      </span>
    </div>
...
function TimeScale({
  ...
  return (
    <div className="min-h-[200px] border-b border-white/[0.06] p-5 last:border-b-0 md:border-b-0 md:border-r md:last:border-r-0 sm:p-6">
      <div
        className="font-mono text-[11px] font-semibold uppercase tracking-[0.09em]"
        style={{ color: `rgba(${rgb},0.60)` }}
      >
        {label}
      </div>
