  ...
  return (
    <div className="flex h-full items-center justify-center font-mono">
      <div className="grid grid-cols-[auto_auto_auto] items-center gap-2 text-[clamp(1.1rem,2.7vw,2.3rem)] font-semibold">
        <span className="rounded-xl border border-violet-200/[0.16] bg-violet-300/[0.08] px-3 py-2 text-violet-100/60">a</span>
        <span className="text-stone-600">+</span>
        <span className="rounded-xl border border-pink-200/[0.16] bg-pink-300/[0.08] px-3 py-2 text-pink-100/60">b</span>
        <span className="col-span-3 text-center text-[14px] text-stone-600">=</span>
        <span className="rounded-xl border border-pink-200/[0.16] bg-pink-300/[0.08] px-3 py-2 text-pink-100/60">b</span>
        <span className="text-stone-600">+</span>
        <span className="rounded-xl border border-violet-200/[0.16] bg-violet-300/[0.08] px-3 py-2 text-violet-100/60">a</span>
      </div>
  ...
  return (
    <div className="flex h-full items-center justify-center font-mono font-semibold">
      <div className="relative flex h-36 w-36 items-center justify-center rounded-full border border-blue-200/[0.16] sm:h-44 sm:w-44">
        <span className="absolute left-4 top-1 text-[20px] text-blue-100/45">ℝ</span>
        <div className="relative flex h-28 w-28 items-center justify-center rounded-full border border-cyan-200/[0.18] sm:h-36 sm:w-36">
          <span className="absolute left-4 top-1 text-[19px] text-cyan-100/50">ℚ</span>
          <div className="relative flex h-20 w-20 items-center justify-center rounded-full border border-green-200/[0.18] sm:h-28 sm:w-28">
            <span className="absolute left-3 top-1 text-[18px] text-green-100/50">ℤ</span>
...
function LessonBackdrop({ slug }: { slug: string }) {
  ...
  return (
    <div aria-hidden="true" className="absolute inset-y-0 right-0 w-full opacity-35 sm:w-[54%] sm:opacity-70">
      {visual}
      <div className="absolute inset-0 z-10 bg-[linear-gradient(90deg,#06140f_0%,rgba(6,20,15,0.82)_18%,transparent_70%)]" />
    </div>
  );
}
          ...
          const surface = (
            ...
            >
              ...
              <span className="relative z-20 grid min-h-[118px] gap-5 sm:grid-cols-[58px_minmax(0,1fr)_auto] sm:items-center sm:pr-[35%] lg:pr-[40%]">
                <span className={`flex h-12 w-12 items-center justify-center rounded-2xl border font-mono text-[12px] font-semibold ${presentation.marker} ${live ? "" : "opacity-45"}`}>
                  {item.label}
                </span>
                <span>
                  <strong className={`block text-[clamp(1.35rem,2.8vw,2rem)] tracking-[-0.04em] ${live ? "text-stone-100" : "text-stone-500"}`}>
                    {item.title}
                  </strong>
                  <span className={`mt-2 block max-w-2xl text-[14px] leading-6 ${live ? "text-stone-400" : "text-stone-600"}`}>
                    {item.description}
                  </span>
                </span>
                <span className={`flex h-10 w-10 items-center justify-center rounded-full border ${presentation.border} ${presentation.text} ${live ? "" : "opacity-35"}`}>
                  {live ? <ArrowRight size={17} aria-hidden="true" /> : <CircleDashed size={15} aria-hidden="true" />}
                </span>
              </span>
            </span>
          );
