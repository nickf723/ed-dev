...
const questions = [
  ...
  { number: 4, prompt: "Which claim is best supported by this document?", options: ["People arrested should be presumed guilty until proven innocent.", "The rights of all citizens should be respected with fairness and justice.", "Religious practices should be limited by the government.", "Freedom of speech should be restricted for the public good."], answer: 1, insight: "All three details establish protected rights with legal boundaries; the broad claim must cover that shared pattern." },
] as const;
  ...
  return <main className="relative min-h-screen overflow-x-hidden bg-[#07101a] text-stone-100">
    ...
    <div className="relative mx-auto w-full max-w-[1050px] px-4 py-5 sm:px-6">
      ...
      <section className="mt-5 rounded-[22px] border border-blue-200/14 bg-black/20 p-5">
        <div className="text-[11px] font-semibold uppercase tracking-[0.14em] text-blue-300/75">Released document · complete excerpt</div>
        <h2 className="mt-2 text-[20px] font-semibold text-white">Declaration of the Rights of Man and of the Citizen · 1789</h2>
        <p className="mt-2 text-[15px] leading-7 text-stone-300">Base your answers to questions 3 and 4 on the document below and on your knowledge of social studies.</p>
        <div className="mt-4 space-y-3">{articles.map((detail, detailIndex) => { const active = evidence.includes(detailIndex); return <button key={detail} type="button" aria-pressed={active} onClick={() => setEvidence((current) => active ? current.filter((value) => value !== detailIndex) : [...current, detailIndex])} className={`w-full rounded-[14px] border p-4 text-left text-[15px] leading-7 ${active ? "border-blue-200/24 bg-blue-300/[0.08] text-blue-50" : "border-white/[0.07] bg-black/15 text-stone-300"}`}><span className="mb-2 block font-mono text-[12px] text-blue-300">{9 + detailIndex}.</span>{detail}</button>; })}</div>
        <div className="mt-3 min-h-[52px] text-[15px] leading-6 text-stone-300">{evidence.length === 3 ? "Pattern: each article limits authority and protects individual rights, though those rights still operate within law." : "Select each paragraph as you read. Collect all three details before generalizing from the document."}</div>
        <p className="mt-2 text-[13px] text-stone-500">Source: Declaration of the Rights of Man and of the Citizen, 1789</p>
      </section>
      ...
      <p className="mt-5 text-[13px] leading-6 text-stone-500">Document text, question wording, and answer order are transcribed from NYSED’s released June 2025 exam. Selection behavior, evidence prompts, and guided explanations are independently authored.</p>
    </div>
  </main>;
}
