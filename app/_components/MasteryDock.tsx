...
const FOUNDATION_NODE_ID = "formal.mathematics.foundations";

export default function MasteryDock() {
  const pathname = usePathname();
  const node = curriculumRegistry.getNodeByHref(pathname);
  const parent = node ? curriculumRegistry.parentFor(node.id) : undefined;

  if (!node || parent?.id !== FOUNDATION_NODE_ID) return null;
  ...
  return (
    <div className="fixed bottom-4 left-4 z-[65] flex items-center gap-3 rounded-2xl border border-white/10 bg-neutral-950/90 p-2.5 pl-3 shadow-[0_16px_45px_rgba(0,0,0,0.45)] backdrop-blur-xl md:bottom-5 md:left-auto md:right-5">
      <div className="hidden sm:block">
        ...
        <div className="max-w-32 truncate text-xs font-semibold text-neutral-200">{node.label}</div>
      </div>
      <MasteryToggle nodeId={node.id} />
    </div>
  );
}
