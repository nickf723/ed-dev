...
import { ArrowLeft, Construction, Waypoints } from "lucide-react";
import { notFound } from "next/navigation";
import { curriculumRegistry } from "@/lib/curriculum/registry";

const LOGIC_PATH = "/formal-science/logic";

type PlaceholderPageProps = {
  params: Promise<{ placeholder: string[] }>;
};

export default async function LogicPlaceholderPage({ params }: PlaceholderPageProps) {
  const { placeholder } = await params;
  const href = `${LOGIC_PATH}/${placeholder.join("/")}`;
  const node = curriculumRegistry.getNodeByHref(href);

  if (!node || node.status !== "placeholder") notFound();

  return (
    <main className="relative min-h-screen overflow-hidden bg-[#05030a] px-6 py-16 text-white selection:bg-purple-500/30">
      <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(rgba(168,85,247,0.035)_1px,transparent_1px),linear-gradient(90deg,rgba(168,85,247,0.035)_1px,transparent_1px)] bg-[size:32px_32px]" />
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_15%,rgba(168,85,247,0.14),transparent_35%),linear-gradient(to_bottom,transparent,rgba(0,0,0,0.6))]" />
      ...
      <div className="relative z-10 mx-auto flex min-h-[70vh] max-w-3xl flex-col justify-center">
        ...
        <section className="relative overflow-hidden rounded-[28px] border border-purple-500/25 bg-black/45 p-8 shadow-[0_24px_80px_rgba(0,0,0,0.5)] backdrop-blur-2xl sm:p-12">
          <div className="absolute right-8 top-8 text-purple-500/10">
            <Waypoints size={120} strokeWidth={1} />
          </div>
          ...
          <div className="relative z-10">
            ...
            <p className="mt-5 max-w-2xl text-base leading-7 text-zinc-400">
              {node.description ?? "This logic branch is mapped into the curriculum, but its learning experience has not been built yet."}
            </p>
