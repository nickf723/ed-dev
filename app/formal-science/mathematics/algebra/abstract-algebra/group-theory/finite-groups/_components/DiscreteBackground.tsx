...
export default function DiscreteBackground() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      // 1. Heavy Fade for subtle trails in deep violet
      ctx.fillStyle = "rgba(9, 2, 17, 0.2)"; 
      ...
      for (let i = 0; i < cols; i++) {
          for (let j = 0; j < rows; j++) {
              ...
              if (val % 11 === 0) {
                  ...
                  const alpha = (Math.sin(time * 0.02 + i * 0.1) + 1) * 0.15; 
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 pointer-events-none z-0 bg-[radial-gradient(circle_at_center,transparent_0%,#090211_95%)]" />
    </>
  );
}