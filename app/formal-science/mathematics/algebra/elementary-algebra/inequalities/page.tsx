...
const NUMBER_EXAMPLES: readonly NumberExample[] = [
  {
    ...
    r: 3,
    relation: "<",
    note: "Start with the simplest idea: values smaller than 3, with 3 excluded.",
  },
  {
    id: "inclusive",
    ...
    r: -2,
    relation: "≥",
    note: "The equality bar means the boundary itself belongs to the solution set.",
  },
  {
    id: "negative",
...
const PLANE_EXAMPLES: readonly PlaneExample[] = [
  {
    ...
    intercept: -1,
    relation: ">",
    note: "Greater-than selects the half-plane above the boundary, excluding the line.",
  },
  {
    id: "below",
  ...
  },
  {
    ...
    intercept: 2,
    relation: "≥",
    note: "A flat boundary makes the above/below interpretation especially clear.",
  },
] as const;
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1540px] px-4 py-4 sm:px-6 xl:px-8 xl:py-5">
        ...
        <section className="mt-3 rounded-[26px] border border-sky-200/[0.12] bg-black/[0.25] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.025),0_28px_80px_rgba(0,0,0,0.22)] backdrop-blur-xl">
          <div className="flex flex-col gap-3 px-1 pb-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-[13px] text-slate-500">Learn from a few canonical cases first. Open the sandbox only when you want to experiment.</p>
            </div>
          ...
          {mode === "number" ? (
            <div className="grid items-start gap-3 xl:grid-cols-[300px_minmax(520px,1fr)_330px]">
              <div className="rounded-[20px] border border-sky-200/[0.08] bg-[#061221]/76 p-4">
                <div className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">Learn the three cases</div>
              ...
              <details className="group rounded-[18px] border border-white/[0.055] bg-black/[0.16] xl:col-span-3">
                ...
                <div className="grid gap-3 border-t border-white/[0.05] p-4 md:grid-cols-2 xl:grid-cols-5">
                    ...
                    <div className="mt-2 grid grid-cols-6 gap-1.5">
                      {COEFFICIENTS.map((value) => (
                        <button
                          key={value}
                          type="button"
                          onClick={() => { setCoefficient(value); setActiveNumberExample(null); }}
                  ...
                  <SliderControl label="Constant" value={constant} min={-5} max={5} step={1} rgb="99, 102, 241" onChange={(value) => { setConstant(value); setActiveNumberExample(null); }} />
                  <SliderControl label="Right side" value={rightSide} min={-5} max={7} step={1} rgb="251, 191, 36" onChange={(value) => { setRightSide(value); setActiveNumberExample(null); }} />
                  <RelationPicker value={relation} onChange={(value) => { setRelation(value); setActiveNumberExample(null); }} />
            ...
            <div className="grid items-start gap-3 xl:grid-cols-[300px_minmax(520px,1fr)_330px]">
              <div className="rounded-[20px] border border-sky-200/[0.08] bg-[#061221]/76 p-4">
                <div className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">Extend the same idea into 2D</div>
                <div className="mt-3 grid gap-2">
                  {PLANE_EXAMPLES.map((example, index) => (
                    <ExampleButton
                      key={example.id}
                      active={activePlaneExample === example.id}
                      number={index + 1}
                      label={example.label}
                      equation={`y ${example.relation} ${formatLine(example.slope, example.intercept)}`}
                      note={example.note}
                      onClick={() => applyPlaneExample(example.id)}
                    />
                  ))}
                </div>
                <div className="mt-3 rounded-[16px] border border-white/[0.045] bg-black/[0.16] p-3.5">
                  <div className="text-[9px] font-semibold uppercase tracking-[0.11em] text-slate-600">Current statement</div>
                  <div className="mt-2 font-mono text-[23px] font-semibold text-white">y <span className="text-sky-300">{planeRelation}</span> {formatLine(planeSlope, planeIntercept)}</div>
                </div>
              </div>
              ...
              <div className="rounded-[20px] border border-indigo-200/[0.09] bg-[#080b1b]/74 p-4">
                ...
                <p className="mt-1 text-[11px] leading-5 text-slate-600">The equality line is the boundary. The inequality chooses a side.</p>
                <div className="mt-3 grid gap-2">
                  <Readout label="1 · Boundary" value={`y = ${formatLine(planeSlope, planeIntercept)}`} rgb="14, 165, 233" />
                  <Readout label="2 · Direction" value={isGreaterRelation(planeRelation) ? "shade above the line" : "shade below the line"} rgb="99, 102, 241" />
                  <Readout label="3 · Inclusion" value={isInclusive(planeRelation) ? "solid boundary · included" : "dashed boundary · excluded"} rgb="251, 191, 36" />
                  <Readout label={`Test (${probeX}, ${probeY})`} value={planeProbePasses ? "inside the region" : "outside the region"} rgb={planeProbePasses ? "52, 211, 153" : "251, 113, 133"} />
                </div>
              </div>

              <details className="group rounded-[18px] border border-white/[0.055] bg-black/[0.16] xl:col-span-3">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-4 py-3.5">
                  <span className="flex items-center gap-2 text-[12px] font-semibold text-slate-400"><SlidersHorizontal size={14} className="text-sky-300/70" /> Explore your own half-plane</span>
                  <span className="text-[10px] text-slate-600 group-open:hidden">optional sandbox</span>
                </summary>
                <div className="grid gap-3 border-t border-white/[0.05] p-4 md:grid-cols-2 xl:grid-cols-5">
                  <SliderControl label="Boundary slope" value={planeSlope} min={-3} max={3} step={0.5} rgb="14, 165, 233" onChange={(value) => { setPlaneSlope(value); setActivePlaneExample(null); }} />
                  <SliderControl label="Boundary intercept" value={planeIntercept} min={-5} max={5} step={1} rgb="244, 114, 182" onChange={(value) => { setPlaneIntercept(value); setActivePlaneExample(null); }} />
                  <RelationPicker value={planeRelation} onChange={(value) => { setPlaneRelation(value); setActivePlaneExample(null); }} />
        ...
        <section className="mt-3 grid gap-3 lg:grid-cols-[minmax(0,0.82fr)_minmax(0,1.18fr)]">
          <div className="rounded-[22px] border border-rose-200/[0.09] bg-black/[0.20] p-4 backdrop-blur-xl">
            <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-rose-300/70">Why multiplying by a negative reverses the sign</div>
...
function ModeButton({ active, label, icon: Icon, onClick }: { active: boolean; label: string; icon: LucideIcon; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className={`flex min-h-10 items-center justify-center gap-2 rounded-xl border px-3 py-2 text-[11px] font-semibold transition-colors ${active ? "border-sky-300/[0.25] bg-sky-400/[0.06] text-sky-200" : "border-white/[0.05] bg-black/[0.16] text-slate-500"}`}>
      <Icon size={14} /> {label}
    </button>
  );
}
...
function ExampleButton({ active, number, label, equation, note, onClick }: { active: boolean; number: number; label: string; equation: string; note: string; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className={`rounded-[15px] border p-3 text-left transition-colors ${active ? "border-sky-300/[0.24] bg-sky-400/[0.055]" : "border-white/[0.045] bg-black/[0.13] hover:border-white/[0.10]"}`}>
      <div className="flex items-center justify-between gap-3">
        <span className={`text-[11px] font-semibold ${active ? "text-sky-200" : "text-slate-400"}`}>{number}. {label}</span>
        <span className="font-mono text-[11px] text-slate-500">{equation}</span>
      </div>
      <p className="mt-1.5 text-[10px] leading-4 text-slate-600">{note}</p>
    </button>
  );
}
...
function SliderControl({ label, value, min, max, step, rgb, onChange }: { label: string; value: number; min: number; max: number; step: number; rgb: string; onChange: (value: number) => void }) {
  return (
    <label className="block rounded-[15px] border border-white/[0.045] bg-white/[0.012] p-3">
  ...
  return (
    <div className="rounded-[15px] border border-white/[0.045] bg-white/[0.012] p-3">
      ...
      <div className="mt-2 grid grid-cols-4 gap-1.5">
        {RELATIONS.map((item) => (
          <button key={item} type="button" onClick={() => onChange(item)} className={`h-9 rounded-lg border font-mono text-[14px] transition-colors ${value === item ? "border-sky-300/[0.28] bg-sky-400/[0.07] text-sky-200" : "border-white/[0.045] bg-black/[0.14] text-slate-500"}`}>{item}</button>
        ))}
      </div>
    </div>
  );
...
function NumberLinePanel({ solution, probe, probePasses }: { solution: ReturnType<typeof solveLinearInequality>; probe: number; probePasses: boolean }) {
  ...
  return (
    <div className="relative flex min-h-[380px] items-center justify-center overflow-hidden rounded-[20px] border border-sky-200/[0.10] bg-[#051321]/82 p-4">
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full max-w-[820px]" aria-label="Number line showing the inequality solution">
        ...
        <line x1={solution.greater ? boundaryX : left} y1={y} x2={solution.greater ? right : boundaryX} y2={y} stroke="#38bdf8" strokeWidth="10" strokeLinecap="round" opacity="0.65" />
  ...
  return (
    <div className="relative flex min-h-[380px] items-center justify-center overflow-hidden rounded-[20px] border border-sky-200/[0.10] bg-[#051321]/82 p-4">
      ...
      <svg viewBox={`0 0 ${size} ${size}`} className="h-full max-h-[445px] w-full max-w-[445px]" aria-label="Coordinate plane with shaded inequality solution region">
        ...
        <line x1={xFor(PLANE_MIN)} y1={yFor(leftY)} x2={xFor(PLANE_MAX)} y2={yFor(rightY)} stroke="#38bdf8" strokeWidth="3" strokeDasharray={isInclusive(relation) ? undefined : "8 7"} />
...
function Readout({ label, value, rgb }: { label: string; value: string; rgb: string }) {
  return (
    <div className="min-h-[52px] rounded-[14px] border border-white/[0.045] bg-white/[0.012] px-3 py-2.5">
      <div className="text-[9px] font-semibold uppercase tracking-[0.1em] text-slate-600">{label}</div>
      <div className="mt-1 text-[11px] leading-4" style={{ color: `rgba(${rgb},0.82)` }}>{value}</div>
    </div>
  );
}
    ...
    <nav className="mt-3 pb-8" aria-label="Integrated Algebra topic navigation">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <Link href="/formal-science/mathematics/algebra/elementary-algebra/systems-of-equations" className="group flex min-h-[74px] items-center gap-3 rounded-[18px] border border-cyan-300/[0.10] bg-black/[0.20] px-4 py-3 transition-colors hover:border-cyan-300/[0.18]">
          ...
          <span><span className="block text-[10px] font-semibold uppercase tracking-[0.11em] text-slate-600">Previous Integrated Algebra topic</span><strong className="mt-0.5 block text-[15px] text-slate-200">Systems of Equations</strong></span>
        </Link>
        <Link href="/formal-science/mathematics/algebra/elementary-algebra/quadratic-equations" className="group flex min-h-[74px] items-center gap-3 rounded-[18px] border border-rose-300/[0.10] bg-black/[0.20] px-4 py-3 transition-colors hover:border-rose-300/[0.18]">
          <span className="min-w-0 flex-1 text-right"><span className="block text-[10px] font-semibold uppercase tracking-[0.11em] text-slate-600">Next Integrated Algebra topic</span><strong className="mt-0.5 block text-[15px] text-slate-200">Quadratic Equations</strong></span>
...
function relationMeaning(relation: Relation) {
  if (relation === "<") return "The left side must be smaller. The equality boundary is excluded.";
  if (relation === "≤") return "The left side may be smaller or exactly equal. The boundary is included.";
  if (relation === ">") return "The left side must be larger. The equality boundary is excluded.";
  return "The left side may be larger or exactly equal. The boundary is included.";
}
...
function formatLine(slope: number, intercept: number) {
  ...
  const mx = slope === 1 ? "x" : slope === -1 ? "−x" : `${formatNumber(slope)}x`;
