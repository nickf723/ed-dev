...
import { 
  ArrowLeft, Variable, Grid3X3, 
  Infinity, Braces, Equal, Divide,
  ChevronRight
} from "lucide-react";

// The newly added imports for the Verification Protocol!
...
const SUBDOMAINS = [
  {
    id: "pre-algebra", title: "Pre-Algebra", subtitle: "Foundations",
    desc: "Integers, order of operations, and the introduction of the variable.",
    icon: Divide, equation: "2(x + 3) = 10",
    ...
    layout: "col-span-12 lg:col-span-7" // Wide
  },
  {
    id: "elementary-algebra", title: "Integrated Algebra", subtitle: "Solving for X",
    desc: "Linear equations, inequalities, graphing, and polynomials.",
    icon: Variable, equation: "ax² + bx + c = 0",
    color: "text-blue-400", border: "group-hover:border-blue-500/50",
    bg: "hover:bg-blue-950/20",
    href: "/formal-science/mathematics/algebra/elementary-algebra",
    layout: "col-span-12 lg:col-span-5" // Narrow
  },
  {
    id: "linear-algebra", title: "Linear Algebra", subtitle: "Vectors & Spaces",
    desc: "Vector spaces, linear mappings, eigenvalues, and matrix theory.",
    icon: Grid3X3, equation: "Ax = λx",
    color: "text-indigo-400", border: "group-hover:border-indigo-500/50",
    bg: "hover:bg-indigo-950/20",
    href: "/formal-science/mathematics/algebra/linear-algebra",
    layout: "col-span-12 lg:col-span-5" // Narrow
  },
  {
    id: "abstract-algebra", title: "Abstract Algebra", subtitle: "Structures",
    desc: "Algebraic structures such as groups, rings, fields, and modules.",
    icon: Infinity, equation: "G/Ker(φ) ≅ Im(φ)",
    color: "text-violet-400", border: "group-hover:border-violet-500/50",
  ...
  return (
    <main className="relative min-h-screen bg-[#0c0a1f] text-white overflow-hidden font-mono selection:bg-indigo-500/50 pb-32">
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto px-6 py-12 min-h-screen flex flex-col">
             ...
             <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-indigo-500/20 pb-8">
                 <div className="flex items-center gap-6">
                     ...
                     <div>
                        <div className="flex items-center gap-3 text-indigo-500 mb-2 font-mono text-[10px] font-bold tracking-[0.2em] uppercase">
                            <Variable size={12} /> Domain_02 <span className="w-12 h-px bg-indigo-500/50"></span>
                        </div>
                 ...
                 <div className="flex gap-4">
                    <div className="bg-black/60 border border-indigo-500/30 rounded-xl p-4 backdrop-blur-sm shadow-xl flex flex-col justify-center">
                        ...
                        <div className="text-2xl font-bold text-white leading-none">04</div>
                    </div>
                 </div>
             </div>
        </header>
        ...
        {/* =========================================
            ZONE 1: THEORY & THE ABSTRACTION STAIRCASE
        ========================================= */}
        <section className="mb-24">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                {/* The "Zig-Zag" Grid Layout */}
                {SUBDOMAINS.map((item, i) => (
                    <Link 
                        key={item.id} href={item.href}
                        className={`
                            ${item.layout}
                            group relative flex flex-col md:flex-row items-start md:items-center gap-6 p-8 border border-white/5 bg-black/40 rounded-2xl backdrop-blur-md transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_10px_30px_rgba(0,0,0,0.5)] hover:border-indigo-500/50 ${item.bg}
                        `}
                    >
                        {/* Brackets decoration */}
                        ...
                        <div className={`p-4 rounded-xl bg-black/60 border border-white/5 shadow-inner transition-colors group-hover:${item.border} ${item.color} shrink-0`}>
                            <item.icon size={32} strokeWidth={1.5} />
                        </div>

                        <div className="flex-1">
                            <div className="flex items-center gap-3 mb-1">
                                <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded bg-white/5 ${item.color}`}>MOD_0{i+1}</span>
                                <h2 className="text-2xl font-bold text-white group-hover:text-indigo-200 transition-colors font-sans">{item.title}</h2>
                            </div>
                            <div className={`text-[9px] font-bold uppercase tracking-widest mb-3 opacity-70 ${item.color}`}>{item.subtitle}</div>
