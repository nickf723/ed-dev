...
function buildSoftwareSectors() {
  return SOFTWARE_SECTORS.map((presentation) => {
    const node = curriculumRegistry.getNode(presentation.nodeId);
    if (!node) throw new Error(`Software curriculum node ${presentation.nodeId} is missing from the registry.`);
    if (!node.description) throw new Error(`Software curriculum node ${presentation.nodeId} is missing its description.`);
...
const SECTORS = buildSoftwareSectors();
type SoftwareSector = (typeof SECTORS)[number];

export default function SoftwareHub() {
  const [activeSnippet, setActiveSnippet] = useState(SECTORS[0].snippet);
